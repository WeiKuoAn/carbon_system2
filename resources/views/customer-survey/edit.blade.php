...
@section('body')
    ...
    @endsection
    @section('content')
    <form action="{{ route('cust.surveys.update',[$customer_id,$survey->id]) }}" method="POST">
    @csrf
    @method('PUT')
    <div class="row">
                                ...
                                @foreach($question_datas as $question_data)
                                    ...
                                    <span class="badge bg-success mb-2 font-size-16">{{ $question_data->face_data() }}</span>
                                        ...
                                        <div class="col-md-5 font-size-22">
                                            ...
                                            @endphp
                                            <div>
                                                {{-- @if($answers_datas->{'ons_' . ($key + 1)}   == $scores[$key]) checked @endif --}}
                                                <div class="form-check mb-3">
                                                    <input class="form-check-input" type="radio" name="formRadios_{{ $question_data->no }}"
                                                        id="formRadios{{ $question_data->no }}_{{ $key }}" value="{{ $scores[$key] }}" 
                                                        @if( $answers_datas->{'ons_' . ($question_data->no)}   == $scores[$key]) checked @endif required>

...
@section('body')
    ...
    @endsection
    @section('content')
    <form action="{{ route('survey.questions.update',$survey_id) }}" method="POST">
    @csrf
    <div class="row">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                @foreach($datas as $key => $data)
                                    <div class="questionDiv mt-5" id="{{ $data->no }}">
                                        <div class="alert alert-success">
                                            <div class="row align-items-center">
                                                <div class="col-md-6 gap-2">
                                                    <h4><span>題號：第{{ $data->no }}題</span></h4>
                                                </div>
                                                <div class="col-md-6">
                                                ...
                                                <input  type="hidden" name="ons[]" value="{{ $data->no }}">
                                            </div>
                                        </div>
                                        <div class="mb-3 row">
                                                ...
                                                @if(count($faces) > 0)
                                                    <select class="form-control" name="face_ids[]" required >
                                                        ...
                                                        @foreach($faces as $face)
                                                            <option value="{{ $face->id }}" @if($data->face_id == $face->id) selected @endif>{{ $face->name }}</option>
                                                        @endforeach
                                                    </select>
                                                @else
                                                    <select class="form-control" name="face_ids[]" required >
                                        ...
                                        <div class="mb-3 row">
                                            ...
                                            <div class="col-md-11">
                                                <select  class="form-control"  name="types[]">
                                                    <option value="single" @if($data->type == 'single') selected @endif>單選</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="mb-3 row">
                                            ...
                                            <div class="col-md-11">
                                                <input  class="form-control"  type="text" name="titles[]" value="{{ $data->title }}" required>
                                            </div>
                                        </div>
                                        <div class="mb-3 row answerOptions">
                                            <label for="example-search-input" class="col-md-1 col-form-label">問卷答案：</label>
                                                <div class="col-md-11">
                                                    @foreach(json_decode($data->options) as $key=>$option)
                                                        @php
                                                        ...
                                                        <div class="hstack gap-3 mb-3">
                                                            <div class="col-md-8">
                                                                <input class="form-control me-3" type="text" placeholder="答案" name="options[{{ $data->no }}][]" value="{{ $option }}">
                                                            </div>
                                                            <div class="col-md-3">
                                                                <input class="form-control me-3" type="text" placeholder="分數" name="scores[{{ $data->no }}][]" value="{{ $scores[$key] }}">
                                                            </div>
                                                            @if($key == 0)
                                                                <div class="col-md-2">
        ...
        <script>
            $(document).ready(function(){
                ...
                function getQuestionCount() {
                    return $('.questionDiv').length;
                }

                $('#addQuestion').click(function(){
                    questionNumber++; // 題號遞增
                    var newQuestionDiv = `
                        <div class="questionDiv mt-5" id="${questionNumber}">
                                    <div class="alert alert-success">
                                        <div class="row align-items-center" >
                                            <div class="col-md-6 gap-2">
                ...
                $('.questionDiv').on('click', '.addAnswerOption', function(){
                    var questionNumber = $(this).closest('.questionDiv').attr('id');
                    var newAnswerOption = `
                        <div class="hstack gap-3 mb-3">
                            <div class="col-md-8">
                                <input class="form-control me-3" type="text" placeholder="答案" name="options[${questionNumber}][]">
                ...
                $('.questionDiv').on('click', '.delQuestion', function(){
